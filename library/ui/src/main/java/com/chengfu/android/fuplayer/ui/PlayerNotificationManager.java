...
public class PlayerNotificationManager {
    ...
    public interface MediaDescriptionAdapter {
         ...
         */
        String getCurrentContentTitle(FuPlayer player);
         ...
         */
        @Nullable
        PendingIntent createCurrentContentIntent(FuPlayer player);
         ...
         */
        @Nullable
        String getCurrentContentText(FuPlayer player);
         ...
         */
        @Nullable
        default String getCurrentSubText(FuPlayer player) {
            return null;
        }
         ...
         */
        @Nullable
        Bitmap getCurrentLargeIcon(FuPlayer player, BitmapCallback callback);
    }
    ...
    public interface CustomActionReceiver {
         ...
         */
        List<String> getCustomActions(FuPlayer player);
         ...
         */
        void onCustomAction(FuPlayer player, String action, Intent intent);
    }
     ...
     */
    public static final String ACTION_PLAY = "com.chengfu.android.fuplayer.play";
    /**
     * The action which pauses playback.
     */
    public static final String ACTION_PAUSE = "com.chengfu.android.fuplayer.pause";
    /**
     * The action which skips to the previous window.
     */
    public static final String ACTION_PREVIOUS = "com.chengfu.android.fuplayer.prev";
    /**
     * The action which skips to the next window.
     */
    public static final String ACTION_NEXT = "com.chengfu.android.fuplayer.next";
    /**
     * The action which fast forwards.
     */
    public static final String ACTION_FAST_FORWARD = "com.chengfu.android.fuplayer.ffwd";
    /**
     * The action which rewinds.
     */
    public static final String ACTION_REWIND = "com.chengfu.android.fuplayer.rewind";
    /**
     * The action which stops playback.
     */
    public static final String ACTION_STOP = "com.chengfu.android.fuplayer.stop";
    /**
     ...
     */
    private static final String ACTION_DISMISS = "com.chengfu.android.fuplayer.dismiss";
    ...
    private final FuPlayer.EventListener playerListener;
     ...
     */
    public final void setPlayer(@Nullable FuPlayer player) {
    ...
    protected NotificationCompat.Builder createNotification(
        ...
        if (Util.SDK_INT >= 21
                ...
                && player.getPlaybackState() == FuPlayer.STATE_READY) {
            builder
     ...
     */
    protected List<String> getActions(FuPlayer player) {
        ...
        if (!timeline.isEmpty() && !player.isPlayingAd()) {
            ...
            enableRewind = window.isSeekable && rewindMs > 0;
            enableFastForward = window.isSeekable && fastForwardMs > 0;
    ...
    protected int[] getActionIndicesForCompactView(List<String> actionNames, FuPlayer player) {
        ...
        int stopActionIndex =
                useStopAction ? actionNames.indexOf(ACTION_STOP) : -1;

        int[] actionIndices = new int[3];
        int actionCounter = 0;
        if (pauseActionIndex != -1 && isPlaying(player)) {
     ...
     */
    protected boolean getOngoing(FuPlayer player) {
        int playbackState = player.getPlaybackState();
        return playbackState != FuPlayer.STATE_IDLE || player.getPlaybackError() != null;
    }

    private void previous(FuPlayer player) {
        Timeline timeline = player.getCurrentTimeline();
        if (timeline.isEmpty() || player.isPlayingAd()) {
            return;
        }
    ...
    private void next(FuPlayer player) {
        Timeline timeline = player.getCurrentTimeline();
        if (timeline.isEmpty() || player.isPlayingAd()) {
            return;
        }
    ...
    private boolean isPlaying(FuPlayer player) {
        return player.getPlaybackState() != FuPlayer.STATE_ENDED
                && player.getPlaybackState() != FuPlayer.STATE_IDLE
                && player.getPlayWhenReady();
    }
    ...
    private static Map<String, NotificationCompat.Action> createPlaybackActions(
        ...
        actions.put(
                ...
                new NotificationCompat.Action(
                        ...
                        context.getString(R.string.fu_play_pause_switch),
        ...
        actions.put(
                ACTION_STOP,
                new NotificationCompat.Action(
                        R.drawable.fu_ic_close,
                        context.getString(R.string.fu_controls_stop_description),
                        createBroadcastIntent(ACTION_STOP, context, instanceId)));
        actions.put(
                ACTION_REWIND,
                new NotificationCompat.Action(
                        R.drawable.fu_ic_fast_rewind,
                        context.getString(R.string.fu_fast_rewind),
                        createBroadcastIntent(ACTION_REWIND, context, instanceId)));
        actions.put(
                ACTION_FAST_FORWARD,
                new NotificationCompat.Action(
                        R.drawable.fu_ic_fast_forward,
                        context.getString(R.string.fu_fast_forward),
                        createBroadcastIntent(ACTION_FAST_FORWARD, context, instanceId)));
        actions.put(
                ACTION_PREVIOUS,
                new NotificationCompat.Action(
                        R.drawable.fu_ic_skip_previous,
                        context.getString(R.string.fu_skip_previous),
                        createBroadcastIntent(ACTION_PREVIOUS, context, instanceId)));
        actions.put(
                ACTION_NEXT,
                new NotificationCompat.Action(
                        R.drawable.fu_ic_skip_next,
                        context.getString(R.string.fu_skip_next),
    ...
    private class PlayerListener implements FuPlayer.EventListener {
    ...
    private class NotificationBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            FuPlayer player = PlayerNotificationManager.this.player;
            if (player == null
                    || !isNotificationStarted
