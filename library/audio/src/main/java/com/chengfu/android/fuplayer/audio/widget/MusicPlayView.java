package com.chengfu.android.fuplayer.audio.widget;
...
import com.chengfu.android.fuplayer.audio.R;
...
public class MusicPlayView extends FrameLayout {
    ...
    public MusicPlayView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        icon = findViewById(R.id.music_controller_icon);

        title = findViewById(R.id.music_controller_title);

        subtitle = findViewById(R.id.music_controller_subtitle);

        previous = findViewById(R.id.music_controller_previous);
        if (previous != null) {
            previous.setOnClickListener(componentListener);
        }

        play = findViewById(R.id.music_controller_play);
        if (play != null) {
            play.setOnClickListener(componentListener);
        }

        pause = findViewById(R.id.music_controller_pause);
        if (pause != null) {
            pause.setOnClickListener(componentListener);
        }

        play2 = findViewById(R.id.music_controller_play2);
        if (play2 != null) {
            play2.setOnClickListener(componentListener);
        }

        pause2 = findViewById(R.id.music_controller_pause2);
        if (pause2 != null) {
            pause2.setOnClickListener(componentListener);
        }

        next = findViewById(R.id.music_controller_next);
        if (next != null) {
            next.setOnClickListener(componentListener);
        }

        seek = findViewById(R.id.music_controller_seek);
        if (seek != null) {
            seek.setOnSeekBarChangeListener(componentListener);
        }

        position = findViewById(R.id.music_controller_position);

        duration = findViewById(R.id.music_controller_duration);

        shuffle = findViewById(R.id.music_controller_shuffle_switch);
        if (shuffle != null) {
            shuffle.setOnClickListener(componentListener);
        }

        repeat = findViewById(R.id.music_controller_repeat_switch);
        if (repeat != null) {
