 ...
 * windows of a {@link FuPlayer}'s {@link Timeline} to the media session queue.
 */
public abstract class TimelineQueueNavigator implements MediaSessionConnector.QueueNavigator {
     ...
     * <p>
     * If the number of windows in the {@link FuPlayer}'s {@link Timeline} exceeds {@code maxQueueSize},
     ...
     * update the queue by calling {@link #getMediaDescription(FuPlayer, int)} again.
     *
     * @param player The current player.
     * @param windowIndex The timeline window index for which to provide a description.
     * @return A {@link MediaDescriptionCompat}.
     */
    public abstract MediaDescriptionCompat getMediaDescription(FuPlayer player, int windowIndex);
    ...
    @Override
    public final void onTimelineChanged(FuPlayer player) {
        publishFloatingQueueWindow(player);
    }

    @Override
    public final void onCurrentWindowIndexChanged(FuPlayer player) {
    ...
    @Override
    public final long getActiveQueueItemId(@Nullable FuPlayer player) {
    ...
    @Override
    public void onSkipToQueueItem(FuPlayer player, ControlDispatcher controlDispatcher, long id) {
        Timeline timeline = player.getCurrentTimeline();
        if (timeline.isEmpty() || player.isPlayingAd()) {
            return;
        }
    ...
    @Override
    public void onSkipToNext(FuPlayer player, ControlDispatcher controlDispatcher) {
        Timeline timeline = player.getCurrentTimeline();
        if (timeline.isEmpty() || player.isPlayingAd()) {
            return;
        }
    ...
    private void publishFloatingQueueWindow(FuPlayer player) {
        Timeline timeline = player.getCurrentTimeline();
        if (timeline.isEmpty()) {
            ...
            if (lastWindowIndex != C.INDEX_UNSET) {
                lastWindowIndex =
                        timeline.getNextWindowIndex(
                                lastWindowIndex, FuPlayer.REPEAT_MODE_OFF, shuffleModeEnabled);
                if (lastWindowIndex != C.INDEX_UNSET) {
                    queue.add(
                            new MediaSessionCompat.QueueItem(
            ...
            if (firstWindowIndex != C.INDEX_UNSET && queue.size() < queueSize) {
                firstWindowIndex =
                        timeline.getPreviousWindowIndex(
                                firstWindowIndex, FuPlayer.REPEAT_MODE_OFF, shuffleModeEnabled);
